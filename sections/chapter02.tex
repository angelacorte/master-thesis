...
This function can be used in other aggregate programs by simply using the interface \texttt{DistanceSensor} as context receiver.
...
The reflection mechanism allows to obtain the contexts passed to the aggregate program.
...
Its functionalities are to manage actions and reactions used within the simulation.
The main action is the \emph{RunCollektiveProgram}, which is used to execute the aggregate program on the device.
It has been implemented in such a way as to evaluate runtime properties that can also be lambda functions, but this led to
performance problems, so a cache system has been implemented to avoid evaluating the properties every time, but only in case
of an update of the system.
