...
%our strategy for alignment
%cerco le fuznc cheh hanno come dfispatcher o extension o argomenti tipo aggregate e le allineo
%piu esempietti
...
First, it takes the current path and the messages received whose path is the same, so that it is possible to evaluate only aligned messages.
...
In the \emph{AggregateContext} class, there are also the functions that are used to align the functions of the aggregate program.
Those functions are used within the compiler plugin to keep track of the stack at runtime.
...
\paragraph{Share}
The \texttt{share} is also implemented with the \emph{yielding support} mechanism.
%The concept is the same as the \texttt{exchange} and
