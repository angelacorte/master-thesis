...
\begin{enumerate}
    ...
    \item \textbf{dsl}: is the core of the project, contains the actual implementation of the logic and the \ac{ac} operators
        and relative tests.
    \item \textbf{compiler-plugin}: used to keep track of the stack at runtime, foreach aggregate program.
\end{enumerate}
...
\begin{itemize}
    ...
    \item \textbf{Message}: is an interface that represents the message exchanged between nodes, its concept will be explained in \Cref{subsec:messages};
...
(\Cref{par:syntax-of-field-calculus}) in terms of \emph{exchange} (\Cref{par:communication-in-xc}).
...
The \emph{exchange} communication is based on \emph{anisotropic} communications, meaning that it does not send the same properties
...
operation of \texttt{share} is based on \emph{isotropic} communication, meaning that the information sent is uniform in all directions
...
The construct \texttt{exchange} provides
    i) access to neighbours' values,
    ii) persistence of information for subsequent executions,
    iii) communication with neighbours, and
    iv) compositional behaviour.

As seen in \Cref{par:communication-in-xc}, the \texttt{exchange} function can send and return the same result, or it
...
function to be computed, as seen in the code snippet \Cref{lst:exchange}, returning a \emph{field}.
...
Consider a network consisting of $n$ devices, all of which are neighbours to each other.
...
neighbours of a local variable, then returns the punctual value evaluated from the transformation (\Cref{lst:share}).
...
It takes as parameter \emph{local}, which can be an expression or a value, and returns a \emph{field} of the same input type (\Cref{lst:neighboring}).
...
and it returns the final value of the computation (\Cref{lst:repeat}).
...
it is better not to send messages to neighbours, also for security and privacy reasons, its implementation will be explained in \Cref{par:repeating}.
...
it has been necessary to create specific classes for the handling of relative messages.
...
created with \emph{Collektive} on the \emph{Alchemist} simulator.
...
but in fact they are two different functions and therefore must be aligned as two different programs.
...
The purpose of \ac{kmp} is to streamline the development process of cross-platform projects by minimising
...
As seen in the code snippet \Cref{lst:collektive}, the \emph{Collektive} class takes as parameters the \emph{localId} of the device,
...
The code seen in \Cref{lst:aggregate} is the implementation of the \texttt{aggregate} function that relies on the \emph{Network}.
...
Those constructs will be explained in detail in the \Cref{subsec:aggregate-context} and \Cref{subsec:aggregate-operators}.
...
and $e_s$ (the value to send) coincide, as illustrated in \Cref{par:communication-in-xc}.
...
This means that it is possible to operate on an initial value, but possibly return a different value, as seen in the code snippet \Cref{lst:sharing}.
...
The \texttt{neighboringViaExchange} construct is used to access the values of the neighbours.
It takes as parameter \emph{local}, which can be an expression or a value, and returns a \emph{field} of the same type (\Cref{lst:neighboringViaExchange}).
...
This method acquires the positions of neighbouring devices and computes the distances between them, as exemplified in code snippet \Cref{lst:distance}.
...
Tasks are configured in the \texttt{build.gradle.kts} file, and the parameters in which the two types of tasks differ are
