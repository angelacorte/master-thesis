...
changing the colour of the nodes based on the distance from the source (seen as a rectangle as in \Cref{fig:gradient}).
...
the source is updated in real-time, as shown in \Cref{fig:gradient-moved}.
\begin{figure}[ht!]
    \centering
...
from a source node to a target node, and the communication should be influenced by obstacles in the space (\Cref{lst:channel-with-obstacles-example}).
