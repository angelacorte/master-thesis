...
It doesn't need any particular setup, as it is a simple state change of the device, in which it sums a value to its state
at each iteration.
%todo

\paragraph{Neighbour Counter}
The neighbour counter has been implemented using the \texttt{neighbouringViaExchange} construct.
As the previous one, no specific setup is required, it simply counts the number of neighbours of each node and
keeps the value in its state.
...
\paragraph{Gradient}
The gradient uses the \texttt{share} construct (based on the \texttt{exchange} construct) to calculate the distance from a
source node and communicate it to the other nodes.
