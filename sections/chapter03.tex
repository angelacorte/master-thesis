...
\paragraph{Gradient}
The gradient in \emph{Collektive} uses the \texttt{share} construct (based on the \texttt{exchange} construct) to
calculate the distance from a source node and communicate it to the other nodes.

%In questo tipo di esperimento è presente piu comunicazione tra i nodi, che devono calcolare la distanza dal nodo sorgente
%e comunicarla agli altri nodi.
%Questo tipo di esperimento è stato scelto per valutare la velocità di esecuzione in un caso leggermente più complesso,
%ma comunque basilare, in cui i nod devono comunicare tra loro e calcolare la distanza dal nodo sorgente.
...
\paragraph{Channel with Obstacles}
Between all those tests, the channel with obstacles is the most important one, as it is the most complex and the one that
requires the most computation.

\begin{figure}[ht!]
    \centering
    \includegraphics[width=0.6\textwidth]{figures/channel-results}
    \caption{Graph of the results for the channel with obstacles benchmark, showing that \emph{Collektive} is $4.43$ times faster
    than \emph{Protelis} and $2.26$ times faster than \emph{ScaFi}.}
...
\begin{table}[ht!]
    ...
    \begin{tabular}{|l|c|c|}
        ...
        Channel with obstacles & 4.43                   & 2.26                \\ \hline
    \end{tabular}
