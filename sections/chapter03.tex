...
For this reason, the experiments have been repeated with a different number of neighbours between nodes, to see if the
performance changes with the amount of communication.
To change the number of neighbours, the simulations have been run with a different linking radius, which is the distance
between nodes that allows them to communicate with each other.

%nei test precedenti, la densità della rete era bassa-media per garantire che i nodi non fossero troppo vicini tra loro
%e in maniera tale da poter esequire i test piu volte
%In the previous tests, the network had a low-medium density to
