...
%Uno dei limiti degli attuali approcci di programmazione collettiva è la gestione dell’eterogeneità.
%Di fatto, il sistema assume che vi sia una certa uniformità di capacità lungo il continuum, o che sia possibile
%astrarla in qualche modo (si veda ad esempio l’approccio a “polverizzazione” [13]).
...
It provides a \emph{domain-specific language (DSL)}, libraries, a simulation environment with a GUI, integrated with the
...
is to handle standard values to provide a simplified setting for DSL embedding.
...
The DSL and standard library of reusable functions are implemented in the \texttt{scafi-
...
\emph{ScaFi} is capable of running aggregate programs on the Java Virtual Machine (JVM) and also on web browser thanks
...
Since \emph{ScaFi} only supports JVM and web-based applications, it is not well-suited for the development of complex distributed systems.
devices and communication technologies, such as thin devices that do not support JVM, which limits the heterogeneity of
devices that can be used in the system.
%The main reasons for the adoption of \emph{ScaFi} are its practical use, extensibility, and the fast use of the \emph{ScaFi
%Web} tool, which allows the rapid prototyping of aggregate programs.
...
%todo cite protelis, Java
\emph{Protelis} is a programming language that implements the \emph{field calculus}' semantics, exposed through a C-like
syntax.
