...
\begin{itemize}
    \item \emph{Stateful field evolution}: the expression $\q{rep}(\q{e_1})\{(\q{x}) \rightarrow \q{e_2}\}$ describes a field evolving in time.
        $\q{e_1}$ represents the initial field value, the function $(\q{x}) \rightarrow \q{e_2}$ declares how the field changes
        at each execution;
    \item \emph{Neighbour interaction}: the expression $\q{nbr}\{e\}$ builds a neighbouring field, a view of the field
        values in the surroundings of each device where neighbours are mapped to their evaluations of \texttt{e};
    \item \emph{Domain partitioning}: the expression $\q{if}(\q{e_0})\q{\{e_1\}}\q{\{e_2}\}$ splits the computational field into
        two non-communicating zones hosting isolated computations: $\q{e_1}$ where  $\q{e_0}$ is \texttt{true} and
        $\q{e_2}$ where  $\q{e_0}$ is \texttt{false}
\end{itemize}
...
$\q{share}(\q{e_1})\{(\q{x}) \rightarrow \q{e_2}\}$
...
In this case, $\q{e_2}$ is responsible for processing the neighbouring field into a local value shared with the
...
$\q{}$
\xc{} is based on a key communication primitive: $\q{exchange}(\q{e_i}, (\q{\underline{n}}) \rightarrow \q{return \ e_r} \ \q{send \ e_s})$
which is evaluated as follows.
i) the device computes the local value from the initial value $\q{e_i}$.
ii) the variable \texttt{\underline{n}} gets substituted with the computed nvalues \texttt{\underline{w}} of the received messages, eventually
 using the local value as default, the exchange returns the computed value $\q{v_r}$ from $\q{e_r}$.
iii) $\q{e_s}$ evaluates to a nvalue $\q{\underline{w}_s}$ consisting of local values to be sent to neighbours,
    that will take their corresponding value from $\q{\underline{w}_s}$ and use it for the execution of their next round.
...
$\q{nbr}(\q{e_i}, \q{e_s}) = \q{exchange}(\q{e_i}, (\q{\underline{n}}) \rightarrow \q{return \ e_r} \ \q{send \ e_s})$,
which means that the value of expression $\q{e_s}$ is sent to neighbours and returns the received values gathered as
\texttt{\underline{n}} with its default, thus providing a view on neighbours' values $\q{e_s}$.
Often the expressions $\q{e_r}$ and $\q{e_s}$ coincide, in this case, the \texttt{exchange} function is
$\q{exchange}(\q{e_i}, (\q{\underline{n}}) \rightarrow \q{retsend \ e})$.
...
\xc{} supports conditional expressions, like $\q{if}(\q{cond})\q{\{e_1\}}\ else\ \q{\{e_2}\}$.
