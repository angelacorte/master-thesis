...
distributed systems.
It is now common for individuals to own multiple computing devices of different types, resulting in technology becoming
...
Managing a single device in a distributed system can be challenging for several reasons, including:
...
Macroprogramming is used to promote collective behaviours, leveraging high-level abstractions and constructs to
...
Coordination models are based on the notion that interactions among multiple independent and autonomous software systems
...
\begin{itemize}
    ...
    \item \emph{Domain partitioning}: the expression $if(e_0)\ \{e_1\}\ \{e_2\}$ splits the computational field into
        two non-communicating zones hosting isolated computations: $e_1$ where the condition $e_0$ is \texttt{true} and
        $e_2$ where the condition $e_0$ is \texttt{false}.
\end{itemize}
...
Thanks to the flexibility of its applications,~\ac{ac} has the potential to facilitate the creation of a more systematic
spectrum for transitioning between cloud and distributed systems.
This approach also embraces the emerging domains of Edge Computing~\cite{8795355} and Fog Computing ~\cite{bdcc2020010}.
...
\begin{itemize}
    ...
    \item \emph{Context-awareness}: aggregate programs utilise sensors, communication driven by neighborhood interactions,
...
locally executed.
The system can continuously react to changes by repeatedly executing rounds, allowing self-adaptation to contextual changes.
...
The most successful solution at the moment involves the use of Cloud Computing (\Cref{fig:cloud-edge-continuum}),
...
Furthermore, the aim is to create an integration of this \ac{dsl} for the \emph{Alchemist} simulator ~\cite{10.1007/978-3-030-78198-9_10},
which serves as a meta-simulator primarily tailored for simulating intricate distributed systems across a diverse range of scenarios.
These scenarios can include swarm robotics ~\cite{aguzzi2024macroswarm}, large-scale sensor networks ~\cite{9927406}, crowd
simulation~\cite{7274429}, path planning, and even the morphogenesis of multi-cellular systems.

With these goals in mind, it is intended to create a programming language that can be executed on any device, regardless of its nature,
and that can communicate with other devices in the network, thus simplifying the development of complex systems.
To do this, it is necessary to rely on technologies and languages that can run on any device.
...
Finally, a performance comparison will be made between the extended language and existing solutions through benchmarking
and simulations.
...
\emph{ScaFi}'s core is based on a variant of the \emph{field calculus} called \emph{FScaFi} ~\cite{10.1007/978-3-030-61470-6_21},
...
It also provides various domain-specific APIs that are interoperable with Java, the \emph{Protelis-Lang} \cite{8064092}.
...
is more limited than \emph{Protelis}' and \emph{ScaFi}'s.
...
However, the more complex syntax of \emph{ScaFi} and \emph{FCPP} may prove easier to learn for programmers already familiar with Scala and \emph{C++}.